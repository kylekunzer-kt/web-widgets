...
// test project branch, this spec cannot pass — see this widget's README.md, "Limitations"
// section, for the documented gap. The selectors below follow the widget's actual rendered
// markup (see src/components/TermChipInput.tsx) so the spec is ready to run as soon as such a
// page exists.
// Skipped (not deleted) because there is no fixture page yet: see the NOTE above and
// README.md's Limitations section. Remove `.fixme` once a test project page exists.
test.describe.fixme("datagrid-multi-text-filter-web", () => {
    ...
    test.describe("multi term filtering", () => {
        test("filters rows by a pasted newline-separated list", async ({ page }) => {
            const filter = page.getByRole("textbox", { name: /search terms/i });
            await filter.click();

            // Drive an actual paste (not typed input) via a constructed DataTransfer, so
            // this exercises TermChipInput's onPaste handler rather than onChange. A
            // newline payload is the case onPaste exists for: `input[type=text]` strips
            // `\r\n` from its value during change-event sanitization, so only a real paste
            // event (which carries the raw clipboard text) can prove newline-separated
            // lists — e.g. a column copied from Excel — are split correctly. This needs no
            // browser clipboard permissions, unlike navigator.clipboard + Ctrl/Cmd+V.
            await filter.evaluate((el, text) => {
